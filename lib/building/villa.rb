class Building
  class Villa < Building
    ...
    def generate_occupants
      ...
      (FAMILY_SIZE..num_occupants).each_with_index { |i| occupants << extended }

      (1..SERVANTS_BY_SIZE[size][:cook].roll).each { occupants << Occupant.new(type: :servant, subtype: :cook) }
      (1..SERVANTS_BY_SIZE[size][:maidservant].roll).each { occupants << Occupant.new(type: :servant, subtype: :maidservant) }
