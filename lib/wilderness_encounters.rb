...
class WildernessEncounters < Thor
  ...
  def domain_encounters(terrain, danger_level, hexes, days = 28)
    ...
    puts "#{DANGER_LEVELS[danger_level].split.first} #{terrain_name.capitalize} with #{hexes} hexes."
    ...
    puts table.render_with MarkdownBorder
  end
  ...
  def roll_rarity(danger_level)
    roll_table(MONSTER_RARITY_BY_DANGER_LEVEL[danger_level], rand(1..20))
  end

  def monster_encounter(terrain, rarity)
    terrain.random_monster(rarity).name
  end
...
WildernessEncounters.start(ARGV)
